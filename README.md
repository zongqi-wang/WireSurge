...
The repository currently contains a native Rust CLI and engine foundation:
...
- `crates/metrics`: runner, worker, report, live load snapshot, and HDR latency models.
...
Each connection (`-c`) keeps many queries in flight (`-q`) and pulls work from one shared, lock-free source so a process-wide `--qps` cap and `--count`/`-l` budget apply across all connections. Human mode prints a target/budget banner, TTY-only live samples, a final summary, and a per-connection table. `--output json` writes one JSON value to stdout and suppresses progress. Ctrl-C and SIGTERM request cooperative Tokio cancellation.

See the [current implementation](./docs/current-implementation.md) and [load engine reference](./docs/load-engine.md) for the complete behavior and limits.

Dependencies use narrow feature sets and intentionally broad direct version requirements, while the committed `Cargo.lock` makes builds reproducible. [`cargo-deny`](./deny.toml) checks advisories, licenses, sources, workspace dependency declarations, and duplicate versions in CI.
