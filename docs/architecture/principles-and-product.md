...
The current CLI uses `clap` and DNS message handling uses NLnet Labs `domain`. The current custom JSON/YAML handling, HTTP wire parser, signal integration, and latency histogram remain replacement work under this tenet. See the [Dependency Policy](../dependency-policy.md).
